...
// --- Artifact colors (T4 icon palette) ---
export const ARTIFACT_COLORS: Partial<Record<Name, string>> = {
...
// --- Types ---
export type Granularity = 'day' | '3day' | 'week' | '2week' | 'month' | '2month' | 'quarter' | 'year';
...
export interface BucketWithTargets {
  key: string;
  label: string;
  tooltip: string;
...
function missionSlots(mission: Mission): number {
  return mission.capacity ?? 0;
}
...
function granularityForSpan(dayCount: number, maxColumns: number): Granularity {
  // Pick the granularity whose bucket count is closest to maxColumns without exceeding it
...
// Epoch anchor for multi-day bucketing (Jan 1, 2020 — arbitrary fixed point)
...
function bucketKeyForDay(day: string, gran: Granularity): string {
  ...
  if (gran === '3day') {
    ...
    const bucketStart = EPOCH.add(Math.floor(daysSinceEpoch / 3) * 3, 'day');
  ...
  if (gran === '2week') {
    ...
    const weeksSinceEpoch = weekStart.diff(EPOCH.startOf('isoWeek'), 'week');
    const bucketStart = EPOCH.startOf('isoWeek').add(Math.floor(weeksSinceEpoch / 2) * 2, 'week');
    return bucketStart.format('YYYY-MM-DD');
  }
...
function bucketLabel(key: string, gran: Granularity, showYear: boolean): string {
  if (gran === 'day' || gran === '3day') {
    const d = dayjs(key);
    return showYear ? d.format('M/D/YY') : d.format('M/D');
  }
  if (gran === 'week' || gran === '2week') {
    const d = dayjs(key);
    return showYear ? d.format('M/D/YY') : d.format('M/D');
  }
  if (gran === 'month' || gran === '2month') {
    ...
    return showYear
      ? `${MONTH_NAMES_SHORT[parseInt(m) - 1]} '${year.slice(2)}`
      : MONTH_NAMES_SHORT[parseInt(m) - 1];
  }
  if (gran === 'quarter') {
...
function bucketTooltip(key: string, gran: Granularity): string {
  ...
  if (gran === 'month') {
    const [year, m] = key.split('-');
    return `${MONTH_NAMES_FULL[parseInt(m) - 1]} ${year}`;
  }
  if (gran === '2month') {
    const [year, m] = key.split('-');
    const mi = parseInt(m) - 1;
    return `${MONTH_NAMES_FULL[mi]} \u2013 ${MONTH_NAMES_FULL[Math.min(mi + 1, 11)]} ${year}`;
  }
  if (gran === 'quarter') {
    const year = key.slice(0, 4);
    const qNum = parseInt(key.slice(6)) - 1;
    return `${MONTH_NAMES_FULL[qNum * 3]} \u2013 ${MONTH_NAMES_FULL[qNum * 3 + 2]} ${year}`;
  }
  return key;
}
...
export function useSensorTargetData(
  ...
  // Full data at daily granularity
  const fullData = computed(() => {
    ...
    for (const mission of missions.value) {
      ...
      if (id == null || id === Name.UNKNOWN || id === 10000) continue;
      totals.set(id, (totals.get(id) || 0) + missionSlots(mission));
      ...
      targetMap.set(id, (targetMap.get(id) || 0) + missionSlots(mission));
    }
    ...
    const targetEntries: TargetEntry[] = [...totals.entries()]
      .map(([id, slots]) => ({ id, slots }));
    return { dayMap, allDays, targetEntries };
  });
  ...
  const sortedTargets = computed(() => {
    const entries = [...fullData.value.targetEntries];
    if (sortByFrequency.value) {
      return entries.sort((a, b) => b.slots - a.slots);
    }
  ...
  // Adaptive bucketing — always produces exactly maxColumns buckets
  ...
  // Buckets with per-target breakdown (for timeline/stream/etc variants)
  ...
  // Zoom to a specific bucket's time range
  const zoomToBucket = (bucket: Bucket) => {
    ...
    const first = all.indexOf(firstDay);
    const last = all.indexOf(lastDay);
    if (first < 0 || last < 0) return;
    // Add a small margin so the bucket isn't edge-to-edge
  ...
  // Resolve target info
  const targetInfo = (id: Name): TargetInfo => ({
    id,
    name: getTargetName(id),
    icon: getImageUrlFromId(id, 64),
    color: ARTIFACT_COLORS[id] ?? FALLBACK_COLOR,
  ...
  // Get slots for a target in a bucket
  const slotsForTarget = (bucket: Bucket, id: Name): number => {
    const { dayMap } = fullData.value;
    let sum = 0;
    for (const day of bucket.days) {
      sum += dayMap.get(day)?.get(id) || 0;
    }
    return sum;
  };

  // Freeze the card height during zoom so it doesn't shift under the cursor.
  // Captured on first zoom event, released when zoom resets to full range.
  const isZoomed = computed(() =>
    clampedStart.value > 0 || clampedEnd.value < allDays.value.length
  );
  ...
  const lockCardHeight = (el: HTMLElement | null) => {
    ...
    if (lockedHeight === null) {
      // Capture the initial (full-range) height
  ...
  // Wheel zoom handler (call from component, pass the card ref)
  const onWheel = (e: WheelEvent, cardEl: HTMLElement | null) => {
    if (!e.ctrlKey && !e.metaKey) return;
    e.preventDefault();

    // Lock height before any state changes
  ...
  const onDragStart = (e: MouseEvent) => {
    ...
    dragStartIdx.value = clampedStart.value;
  };
  ...
  const onDragMove = (e: MouseEvent, containerEl: HTMLElement | null) => {
    ...
    const newStart = Math.max(0, Math.min(dragStartIdx.value + dayDelta, total - span));
